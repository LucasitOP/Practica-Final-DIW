...
const clearExpiredEntries = (store, now) => {
...
const checkLocalRateLimit = (ip) => {
    const now = Date.now();
    clearExpiredEntries(localRateLimitStore, now);
...
const sendEmail = async ({ nombre, email, empresa, mensaje, ip }) => {
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const cleanEmpresa = String(empresa || '').trim();
        ...
        if (cleanNombre.length > 120 || cleanEmpresa.length > 120 || cleanMensaje.length > 2000) {
